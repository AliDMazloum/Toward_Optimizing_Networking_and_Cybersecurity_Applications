...
construct_trace_matrix_gpu_low(uint32_t *traceMatrix, const char* signatures,int32_t *report){
    ...
    for(int j =1; j< MaxSignatureLength+1;j++){
        ...
        // match * (PktPayload_d_constant[0] == thread_signature[j])
        // + ((match<<16) * (PktPayload_d_constant[0] == thread_signature_1[j]));
        
        
    }
    
    // u_int32_t iter_index = global_thread_id + row_size;
    ...
    // unsigned short int temp_hi, temp_low;
...
int main(int argc, char* argv[]) {
    ...
    *NumberOfSignatures_h = NumberOfSignatures;
    ...
    // construct_trace_matrix_cpu(traceMatrix, PktPayload, Signatures,report);
