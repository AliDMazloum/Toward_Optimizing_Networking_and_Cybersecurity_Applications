...
u_int8_t signature_matching_score(char* s, int len, int p){
    u_int8_t count = 0;
...
int* input_payloadSize, int* input_maxSignatureLength,int* input_numberOfSignatures, u_int8_t* signaturesMatchingSocres){
    ...
    // char thread_signature[*input_maxSignatureLength+1];
    // char thread_signature_1[*input_maxSignatureLength+1];

    // for(int j =1; j< *input_maxSignatureLength+1;j++){
    //     thread_signature[j] = signatures[offset + j];
    //     thread_signature_1[j] = signatures[offset + j];      
    ...
    for(int64_t i = 1; i < *input_payloadSize+1; i ++){
        ...
        for(int64_t j = 1; j < *input_maxSignatureLength+1; j ++){
            ...
            // if (traceMatrix[offset + i*(*input_maxSignatureLength+1)**input_numberOfSignatures +j]>=signaturesMatchingSocres[global_thread_id]){
            //     report[0] = traceMatrix[offset + i*(*input_maxSignatureLength+1)**input_numberOfSignatures +j];
            //     report[1] = global_thread_id;
            //     report[2] = i-1;
            // }

            current_value = max(max(max(north+(signatures[offset +j-2] != '*')*indel, west+indel), north_west+
                                     (signatures[offset +j-2] != '*')*(signatures[offset +j-2] != '.')*(signatures[offset +j-2] != '~')*
                                     ((payload[i-1] == signatures[offset +j-2])*match + (payload[i-1] != signatures[offset +j-2])*mismatch) +(signatures[offset +j-2] == '~')*
                                     mismatch*(!dev_isdigit(payload[i-1]))),0);
...
int main(int argc, char* argv[]) {
    ...
    if(gridSize ==1){
        gridSize = (input_numberOfSignatures + blockSize - 1) / blockSize;
        grid.x = gridSize;
    }
    ...
    // char singatureRegex[] = "goo.leM*l.ci*c~m";
    ...
    size_t traceMatrixSize = sizeof(int16_t) * signaturesTotalBytes* 3;
